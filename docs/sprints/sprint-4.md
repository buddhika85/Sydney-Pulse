# Sprint 4 — Polish & Portfolio  
**Target:** 6–7 days  
**Goal:** ADRs, cost model, final README, MkDocs site, blog post, demo video, LinkedIn launch.

## Sprint Backlog

| # | Title | Description | Days |
|---|-------|-------------|------|
| 1 | ADRs | Write 8–10 ADRs | 2 |
| 2 | Cost model | Update docs/cost-model.md | 0.5 |
| 3 | Landing page polish | Full design + timeline | 1 |
| 4 | README finalization | Architecture, badges, links | 0.75 |
| 5 | MkDocs site | Deploy docs site to GitHub Pages | 0.5 |
| 6 | Advanced Security | Dependabot, CodeQL, secret scanning | 0.5 |
| 7 | Blog post | 1500–2000 word deep dive | 1 |
| 8 | Final Loom walkthrough | 5-minute demo | 0.5 |
| 9 | LinkedIn launch | Tag v1.0.0, release notes | 0.5 |
| 10 | Code review + interview-prep quiz (always-on) | Sprint-long discipline per `CLAUDE.md` "Daily rhythm". Polish-phase ratio (~40% prep); verbal mock interviews and architecture-explanation rehearsal weighted heavier than new content. | sprint-long |

**Total:** ~7 days dev + sprint-long always-on quiz discipline (SP4-10)

## Deliverables
- Full documentation site  
- Final README  
- Blog post  
- Demo video  
- v1.0.0 release  
- LinkedIn launch  

## Acceptance Criteria
- Docs site live  
- README polished  
- ADRs complete  
- Blog post published  
- Demo video recorded  
